CREATE OR REPLACE FUNCTION public.settle_deferred_sale(_sale_id uuid, _amount_received numeric, _amount_returned numeric DEFAULT 0, _credit_note_id uuid DEFAULT NULL::uuid, _credit_note_issued_amount numeric DEFAULT 0)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  UPDATE public.sales
    SET status = 'completed',
        ...
        credit_note_issued_amount = COALESCE(_credit_note_issued_amount, 0),
        date = date
    WHERE id = _sale_id;
END;
$function$;